        ...
        if platform.system() == "Windows":
            ...
            if os.path.exists(vcpkg_installation_root):
                check_and_install_packages(
                    packages=["curl[ssl]", "eigen3"], triplet="x64-windows-static", vcpkg_root=vcpkg_installation_root
                )
                cmake_args += ["-DLIBCZI_BUILD_PREFER_EXTERNALPACKAGE_LIBCURL=ON"]  # instruct to use the package-manager provided libcurl
                cmake_args += ["-DLIBCZI_BUILD_PREFER_EXTERNALPACKAGE_EIGEN3=ON"]   # instruct to use the package-manager provided eigen3
                cmake_args += [
            ...
            if os.path.exists(vcpkg_installation_root):
                print(subprocess.run("/usr/bin/sh -c 'id&&pwd&&ls -lisa&&ls -lisa /project'"))
                check_and_install_packages(
